...
class RDMuscle(bpy.types.PropertyGroup):
    ...
    # define a variable for muscle dimension
    ...
    '''
    def muscle_dimension_update(self, context):
        active_muscle = global_properties.active_muscle.get(bpy.context.scene)
        bpy.data.objects[active_muscle].data.bevel_depth = context.scene.RobotDesigner.muscle_dim

    dimension = FloatProperty(name="Muscle Dimension", default=0.05, update=muscle_dimension_update)
    '''
