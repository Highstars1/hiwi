...
def draw(layout, context):
    ...
    if box:
        ...
        dimensionbox.label("Change Muscle Dimension:")
        row3 = dimensionbox.row()
        #row3.prop(bpy.data.objects[active_muscle].RobotEditor.muscles, 'Muscle Dimension', text='Muscle Dimension')
    ...
    row1 = box.row()
    # selective display muscle type
    row1.label("Change Muscle Dimension:")

    #define a class for choosing the muscle
    row2 = box.row()
    row2.menu(menus.MuscleMenu.bl_idname, text=active_muscle if not active_muscle == '' else "Select Muscle")
    #box = layout.box()
    row3 = box.row()
    global_properties.muscle_dim.prop(context.scene, row3, expand=True)
